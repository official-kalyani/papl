        ...
        //if(Yii::$app->user->identity->role_id==1 || Yii::$app->user->identity->role_id==3){
            ...
            $states = ArrayHelper::map(State::find()->where(['is_delete' => 0])->all(), 'state_id', 'state_name');
                ...
                if ($model->load(Yii::$app->request->post())) {
                    ...
                    if($filename){
                        $tmp_name = 'Aadhaar_Photo';
                        $file= $model->imageupload($filename,$tmp_name);
                        $model->browse_adhar=$file;
                    }
                    $pp_photo = UploadedFile::getInstance($model, 'browse_pp_photo');

                    if($pp_photo){
                        $pp_name = 'Passport_Photo';
                        $pp_photo_file= $model->imageupload($pp_photo,$pp_name);
         ...
         // echo '<pre>';print_r($this->request->post());
            ...
            if ($userData > 0) {
                ...
                for ($i=0; $i < $userData; $i++) { 
                    ...
                    {
                          // echo "<pre>";print_r($this->request->post()['Qualification']);die();
                            $qualification_model = new Qualification();
                            ...
                            $qualification_model->highest_qualification=$highest_qualification;
                             ...
                             // $qualification_documents = UploadedFile::getInstances($qualification_model, 'qualification_document'.$i);
                            
                               // echo "<pre>";var_dump($qualification_documents);
                                if (isset($qualification_documents)) {
                                // if (isset($qualification_documents[$i])) {

                                    $basepath = Yii::getAlias('@storage');
                                    $randnum = Yii::$app->security->generateRandomString();
                                    $file = '/upload/' .'Qualification Document'.$randnum .'.'. $qualification_documents[$i]->extension;
                                    $path = $basepath . $file;
                                    $qualification_documents[$i]->saveAs($path);
                                    $qualification_model->qualification_document=$file;
                                }else{
                                    if(isset($this->request->post()['Qualification']['qualification_document_old'][$i])){
                                        // echo 'x';die();
                                        $qualification_model->qualification_document=$this->request->post()['Qualification']['qualification_document_old'][$i];
                                    }
                                }
	...
	public function actionInternal()
    {
        ...
        // var_dump($this->request->post());
         if ($model->load($this->request->post())) {
                ...
                // print_r($type);exit();
                if($filename){  
                    $name_file = 'PAPL_Ineternal_Experience';                   
                    $file= $model->fileupload($filename,$name_file);
	...
	public function actionEpf()
    {
        ...
        $model->scenario = 'epf_esic';
         ...
         if ($model->load($this->request->post())) {
                ...
                if($uan_sheet){
                    $tmp_name = 'UAN_Sheet';
                    $filesheet= $model->fileupload($uan_sheet,$tmp_name);
                    $model->uan_sheet=$filesheet;
                }   
                $esic_sheet = UploadedFile::getInstance($model, 'esic_sheet');
                if($esic_sheet){
                    $tmp_name = 'ESIC_Sheet';
                    $file= $model->fileupload($esic_sheet,$tmp_name);
        ...
        if ($bank_model->load($this->request->post())) {
          if (Yii::$app->request->get('papl_id')) {
            ...
            if (empty(($bank_model))) {
                ...
                if($pass_book_photo){
                    $tmp_name = 'pass_book_photo';
                    $file= $bank_model->fileupload($pass_book_photo,$tmp_name);
   ...
   public function actionDocument()
    {
        ...
        if ($document_model->load($this->request->post())) {
            ...
            if (empty($document_model)) {
                ...
                if($voter_copy_photo){
                    $tmp_name = 'voter_copy_photo';
                    $file= $document_model->fileupload($voter_copy_photo,$tmp_name);
                ...
                if($drivinglicense_photo){
                    $tmp_name = 'drivinglicense_photo';
                    $file= $document_model->fileupload($drivinglicense_photo,$tmp_name);
                    $document_model->drivinglicense_photo=$file;
                }   
                $pan_photo = UploadedFile::getInstance($document_model, 'pan_photo');
                if($pan_photo){
                    $tmp_name = 'pan_photo';
                    $file= $document_model->fileupload($pan_photo,$tmp_name);
                    $document_model->pan_photo=$file;
                }   
                $passport_photo = UploadedFile::getInstance($document_model, 'passport_photo');
                if($passport_photo){
                    $tmp_name = 'passport_photo';
                    $file= $document_model->fileupload($passport_photo,$tmp_name);
        ...
        $nominee_model = new Nominee();
         if ($nominee_model->load($this->request->post())) {
                ...
                if($nominee_adhar_photo){
                    $tmp_name = 'nominee_adhar_photo';
                    $file= $nominee_model->imageupload($nominee_adhar_photo,$tmp_name);
                    ...
                    if(Yii::$app->request->post()['Family']['family_member'][$i] !='' && isset(Yii::$app->request->post()['Family']['family_member'][$i]) && !empty(Yii::$app->request->post()['Family']['family_member'][$i]))
                    {
                        // echo "<pre>";print_r($this->request->post()['Family']);
                            ...
                            if (!empty($family_nominee_adhar_photos[$i]) && isset($family_nominee_adhar_photos[$i])) {
                                $basepath = Yii::getAlias('@storage');
                                $randnum = Yii::$app->security->generateRandomString();
                                $file = '/upload/' .'family_nominee_adhar_photo'.$randnum .'.'. $family_nominee_adhar_photos[$i]->extension;
                                $path = $basepath . $file;
